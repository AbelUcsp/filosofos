...
/*
void f(std::mutex forks_[2], int i){

    forks_[i].lock();
      std::cout << " hay forks " << i << std::endl;
...
/*
  //DiningPhilosophers *mesa = new DiningPhilosophers();
  std::thread *A = new std::thread( Philosopher(0, "Belo", kNumberActions, mesa), 11);
  std::thread *B = new std::thread( Philosopher(0, "Abel", kNumberActions, mesa), 11);
  A->join();
  B->join();
  //A->operator()();  
*/
